...
namespace Nekos.SpecialtyPlugin.Commands
{
  ...
  static class CommandMiscellaneous
  {
    ...
    {
      await currentContext.Actor.PrintMessageAsync(
        "<specialty> parameter can be:",
  ...
  public class ReloadConfigCommand : UnturnedCommand
  {
    ...
    public ReloadConfigCommand(SpecialtyOverhaul plugin, IServiceProvider serviceProvider) : base(serviceProvider)
    {
      _plugin = plugin;
    }
    ...
    {
      await Task.Run(_plugin.RefreshConfig);
    }
  }
  ...
  public class GetLevelCommand : UnturnedCommand
  {
    ...
    public GetLevelCommand(SpecialtyOverhaul plugin, IServiceProvider serviceProvider) : base(serviceProvider)
    {
      _plugin = plugin;
    }
  ...
  public class GivePlayerLevelCommand : UnturnedCommand
  {
    ...
    public GivePlayerLevelCommand(SpecialtyOverhaul plugin, IServiceProvider provider) : base(provider)
    {
      _plugin = plugin;
    }
    ...
    protected override async UniTask OnExecuteAsync()
    {
      try
      {
        await CommandMiscellaneous.ParseParameter1(_plugin, Context, (UnturnedUser user, EPlayerSpeciality spec, byte skillidx, int level) =>
        {
          _plugin.SkillUpdaterInstance.GivePlayerLevel(user.Player, (byte)spec, skillidx, level);
        });
      }
      catch(Exception e)
      {
  ...
  public class SetPlayerLevelCommand : UnturnedCommand
  {
    ...
    public SetPlayerLevelCommand(SpecialtyOverhaul plugin, IServiceProvider provider) : base(provider)
    {
      _plugin = plugin;
    }
    ...
    protected override async UniTask OnExecuteAsync()
    {
      try
      {
        await CommandMiscellaneous.ParseParameter1(_plugin, Context, (UnturnedUser user, EPlayerSpeciality spec, byte skillidx, int level) =>
        {
          _plugin.SkillUpdaterInstance.SetPlayerLevel(user.Player, (byte)spec, skillidx, level);
        });
      }
      catch(Exception e)
      {
  ...
  public class GetPlayerLevelCommand: UnturnedCommand
  {
    ...
    public GetPlayerLevelCommand(SpecialtyOverhaul plugin, IServiceProvider provider) : base(provider)
    { 
      _plugin = plugin;
    }
