...
namespace Nekos.SpecialtyPlugin
{
  public class SpecialtyOverhaul : OpenModUnturnedPlugin
  {
    ...
    public SpecialtyOverhaul(
      IConfiguration configuration,
      IStringLocalizer stringLocalizer,
      ILogger<SpecialtyOverhaul> logger,
                ...
                case SkillConfig.EOnDiedEditType.BASE:
                  {
                    ...
                    int _delta = 
                      _calculateLevelBorderExp(ref expData, (EPlayerSpeciality)i_specs, (byte)i_skill, (byte)(_level+1))-
                      _calculateLevelBorderExp(ref expData, (EPlayerSpeciality)i_specs, (byte)i_skill, _level);
    ...
    public KeyValuePair<string, string> GetExp_AsProgressBar(UnturnedPlayer player, EPlayerSpeciality speciality, int skill_idx, bool getskillname = false, bool getspecname = false)
    {
      try
      {
        ...
        return new KeyValuePair<string, string>(
          _strname,
          string.Format("Lvl {0} [{1}] {2}%", player.Player.skills.skills[(int)speciality][skill_idx].level, _strbar, (_range * 100).ToString("F1"))
        );
      }
      catch(Exception e)
      {
